                ...
                <div className=' bg-[#F4F5F9]'>
                    {data.map((item) => (
                        <ul onClick={() => handleOpenEmail(item.id)} key={item.id} className='pl-5 pr-5  pb-5 bg-[#F4F5F9] text-[#636363]'>
                            <li className={`${item.id == selectedId ? 'border-[#E54065]':'border-[#CFD2DC]'} ${item.isRead ? 'bg-[#F4F5F9]':'bg-[#F2F2F2]'} flex border-2  gap-5 p-5 rounded-md  hover:border-[#E54065]  transition-all ease-in-out duration-500`}>
